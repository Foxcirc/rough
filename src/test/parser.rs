...
fn basic_parse() {
    ...
    assert_eq!(parser::parse_ident(ParseInput::new_extra("foo", 0)).map(scnd), Ok(Span { id: 0, start: 0, end: 3 }));
    assert_eq!(parser::parse_ident(ParseInput::new_extra("-foo", 0)).map(scnd), Ok(Span { id: 0, start: 0, end: 4 }));
    assert_eq!(parser::parse_ident(ParseInput::new_extra("foo-bar", 0)).map(scnd), Ok(Span { id: 0, start: 0, end: 7 }));
    assert_eq!(parser::parse_ident(ParseInput::new_extra("foo123", 0)).map(scnd), Ok(Span { id: 0, start: 0, end: 6 }));
